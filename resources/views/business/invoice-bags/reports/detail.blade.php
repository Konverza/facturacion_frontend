...
<head>
    <meta charset="utf-8" />
    <title>Detalle Bolsón {{ $bag->bag_code }}</title>
    <style>
        body {
...
<body>
    ...
    <h1 class="text-center mb-8">Detalle de Productos facturados en Bolsón</h1>
        ...
        <tbody>
            @foreach ($bagInvoices as $invoice)
                @foreach ($invoice->products ?? [] as $product)
                    @php
                        $qty = (float) ($product['cantidad'] ?? 0);
                        $unit = (float) ($product['precio_unitario'] ?? ($product['precio'] ?? 0));
                        $lineTotal = (float) ($product['total'] ?? $qty * $unit);
                    @endphp
                    <tr>
