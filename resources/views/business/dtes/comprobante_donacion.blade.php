...
@section('title', 'Generar comprobante de donación')
@section('content')
    <section class="my-4 pb-4">
        @include('layouts.partials.business.dte.header', [
            'title' => 'Generar comprobante de donación',
        ])
        <form action="{{ Route('business.dte.comprobante-donacion') }}" method="POST">
            @csrf
            ...
            <div class="mt-4">
                        ...
                        <div id="default-styled-tab-content">
                            <div class="hidden" id="styled-receptor" role="tabpanel" aria-labelledby="receptor-tab">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="mt-4 flex flex-col gap-4 sm:flex-row">
                                        <div class="flex-1" id="select-domiciliado">
                                            <x-select name="cod_domiciliado" id="cod_domiciliado" label="Domicilio Fiscal"
                                                value="{{ old('cod_domiciliado', '') }}"
                                                selected="{{ old('cod_domiciliado', '') }}"
                                                :options="['1' => 'Domiciliado', '2' => 'No Domiciliado']" :search="false" required />
                                        </div>
                                        <div class="flex-1" id="select-pais">
                                            <x-select label="País" name="codigo_pais" id="codigo_pais" :options="$countries"
                                                ...
                                                selected="{{ old('codigo_pais', isset($dte['customer']) ? $dte['customer']['codPais'] ?? '' : '') }}" required/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                @include('layouts.partials.business.dte.sections.section-otra-informacion', [
                    ...
                    'venta_cuenta_terceros' => false,
                    'otros_documentos_asociados' => true,
                    'medico' => false,
                ])
                <!-- End Sección datos del emisor y receptor -->
                <!-- Secctosetalles de la factura -->
                <div class="mt-4 border-t border-gray-300 px-4 pt-4 dark:border-gray-800">
                    <h2 class="flex items-center gap-1 text-xl font-semibold text-primary-500 dark:text-primary-300">
                        <x-icon icon="info-circle" class="size-5" />
                        Datos del Comprobante de Donación
                    </h2>
                    <div class="mt-4 flex flex-col items-center justify-start gap-4 sm:flex-row">
                        <div class="relative w-full sm:w-auto">
                            <x-button type="button" icon="plus" typeButton="info" text="Agregar donación"
                                class="show-drawer w-full sm:w-auto" size="normal" data-target="#drawer-new-product"
                                aria-controls="drawer-new-product" />
                        </div>
                    </div>
                    <div class="mt-4" id="table-comprobante-donacion">
                        @include('layouts.partials.ajax.business.table-comprobante-donacion')
                    </div>
                </div>
    ...
    @include('layouts.partials.business.dte.drawer-new-donacion')
@endsection
