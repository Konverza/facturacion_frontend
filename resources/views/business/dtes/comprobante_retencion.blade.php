    ...
    @section('content')
                    ...
                    <div class="mt-4 border-t border-gray-300 px-4 pt-4 dark:border-gray-800">
                        ...
                        @if (!$default_pos)
                            <div
                                ...
                                <b>Nota: </b> No tiene un punto de venta predeterminado, por favor seleccione uno en la
                                pestaña
                                ...
                                <div class="hidden" id="styled-receptor" role="tabpanel" aria-labelledby="receptor-tab">
                                    ...
                                    <div class="mt-4">
                                        <div class="flex flex-col gap-4 sm:flex-row">
                                            ...
                                            <div class="flex-1">
                                                <x-input type="text" label="Número documento"
                                ...
                                <div class="options absolute right-0 top-0 z-10 mt-1 hidden w-calc-full-minus-8 rounded-lg border border-gray-300 bg-white p-1 dark:border-gray-800 dark:bg-gray-950 sm:w-max"
                                    ...
                                    <ul class="flex flex-col text-sm">
                                        ...
                                        <li>
                                            <button type="button"
                                                class="show-modal flex w-full items-center gap-1 rounded-lg px-2 py-2 text-gray-600 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-900"
                                                data-target="#add-documento-hacienda">
                                                <x-icon icon="devices" class="h-4 w-4" />
                                                Electrónico
                                            </button>
                                        </li>
                    ...
                    <form action="{{ Route('business.dte.documents.store') }}" method="POST" class="flex flex-col">
                        ...
                        <div class="flex flex-col gap-4 p-4">
                            ...
                            <div class="flex flex-col items-center gap-4 sm:flex-row">
                                <div class="sm:flex-1 w-full">
                                    <x-input type="text" label="Número de documento" required
                                        id="numero_documento_fisico" placeholder="Ingresar el número del documento"
                            ...
                            <x-input type="number" name="monto_sujeto_retencion" label="Monto sujeto a retención"
                                required icon="currency-dollar" id="monto_sujeto_retencion" step="0.01"
                                placeholder="0.00" />
                            <div class="flex flex-col items-center gap-4 sm:flex-row">
                                <div class="sm:flex-[2] w-full">
                                    <x-select label="Código de tributo" :options="[
                                        ...
                                        value="22" selected="22" name="codigo_tributo" required />
                                </div>
                                        ...
                                        <div class="flex flex-col items-center gap-4 sm:flex-row">
                                            <div class="flex-[2]">
                                                <x-select label="Código de tributo" :options="[
                                                    ...
                                                    id="codigo-tributo-2" value="22" selected="22" required
                                                    name="codigo_tributo" :search="false" />
                                            </div>
                                            <div class="flex-1">
        ...
        <!-- Modal add documento from Hacienda -->
        <div id="add-documento-hacienda" tabindex="-1" aria-hidden="true"
            class="fixed left-0 right-0 top-0 z-[100] hidden h-full max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-200/50 dark:bg-gray-900/50 md:inset-0">
            <div class="relative max-h-full w-full max-w-[950px]">
                <!-- Modal content -->
                <div class="motion-preset-expand relative rounded-lg bg-white shadow motion-duration-300 dark:bg-gray-950">
                    <div class="flex flex-col">
                        ...
                        <div class="flex flex-col gap-4 p-4">
                            ...
                            <div
                                ...
                                <b>Nota: </b> El DTE ya debe haber sido transmitido por su proveedor y el NIT debe coincidir con el de su empresa.
                            </div>
                            <form action="{{ route('business.dte.documents.queryDTE') }}" method="POST"
                                id="form-query-dte">
                                @csrf
                                <div class="flex flex-col gap-4 sm:flex-row">
                                    <div class="flex-[2]">
                                        <x-input type="text" label="Código de Generación del DTE"
                                            name="codigoGeneracion" placeholder="Ingresar el código de generación"
                                            id="codigo-generacion" value="{{ old('codigoGeneracion', '') }}" required />
                                    </div>
                                    <div class="flex-1">
                                        <x-input type="date" id="fechaEmi" name="fechaEmi" label="Fecha de emisión"
                                            required />
                                    </div>
                                </div>
                                <div class="flex mt-2 justify-center">
                                    <x-button type="button" class="submit-form" text="Consultar" icon="search"
                                        typeButton="primary" />
                                </div>
                            </form>
                        </div>
                        <div class="px-4 pb-4">
                            <div id="container-data-hacienda" class="hidden">
                                <form action="{{ Route('business.dte.documents.store-hacienda') }}" method="POST">
                                    @csrf
                                    <div class="mt-4 flex flex-col gap-4">
                                        <input type="hidden" id="cod-generacion-hacienda" name="cod_generacion">
                                        <input type="hidden" name="tipo_dte" id="tipo-dte-hacienda">
                                        <input type="hidden" name="fecha_emision" id="fecha-emision-hacienda">
                                        <x-input type="textarea" label="Descripción" name="descripcion_retencion"
                                            id="descripcion-retencion-hacienda"
                                            placeholder="Ingresar la descripción del documento" required />
                                        <x-input type="number" name="monto_sujeto_retencion"
                                            label="Monto sujeto a retención" icon="currency-dollar"
                                            id="monto-documento-hacienda" step="0.01" placeholder="0.00" required />
                                        <div class="flex flex-col items-center gap-4 sm:flex-row">
                                            <div class="flex-[2]">
                                                <x-select label="Código de tributo" :options="[
                                                    '22' => 'Retención IVA',
                                                    ...
                                                    id="codigo-tributo-hacienda" value="22" selected="22" required
                                                    name="codigo_tributo" :search="false" />
                                            </div>
                                            <div class="flex-1">
                                                <x-input type="number" name="iva_retenido"
                                                    id="iva-retenido-documento-hacienda" label="Monto de IVA"
                                                    placeholder="0.00" step="0.01" icon="currency-dollar" required />
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div
                            class="flex items-center justify-end gap-4 rounded-b border-t border-gray-300 p-4 dark:border-gray-800">
                            <x-button type="button" class="hide-modal" text="Cancelar" icon="x"
                                typeButton="secondary" data-target="#add-documento-hacienda" />
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- End Modal add add documento from Hacienda -->
