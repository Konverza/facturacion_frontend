export function showAlert(type, title, message) {
    ...
    const currentAlertId = `toast-${alertCount++}`;
    const div = $(`<div id="${currentAlertId}" role="alert"></div>`);
    div.addClass(
        "alert flex w-full sm:max-w-sm animate-fade-left overflow-hidden rounded-lg bg-white shadow-md animate-duration-300 dark:bg-gray-900 sm:right-10"
    );
        ...
        <div class="-mx-3 px-4 py-2 flex items-center justify-between w-full">
            <div class="mx-3">
             ...
             <button type="button"
                ...
                data-dismiss-target="#${currentAlertId}" aria-label="Close">
    ...
    $(`button[data-dismiss-target="#${currentAlertId}"]`).click(function () {
