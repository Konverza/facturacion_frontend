...
if (localStorage.getItem("items_nd")) {
    items = JSON.parse(localStorage.getItem("items_nd"));
    cargar_items();
    calcular_tributos_dte();
    calcular_totales();
}
if (localStorage.getItem("reteIva1_nd")) {
    reteIva1 = parseFloat(localStorage.getItem("reteIva1_nd"));
...
if (localStorage.getItem("perciIva1_nd")) {
    perciIva1 = parseFloat(localStorage.getItem("perciIva1_nd"));
...
if (localStorage.getItem("reteRenta_nd")) {
    reteRenta = parseFloat(localStorage.getItem("reteRenta_nd"));
...
$(function () {
    ...
    $("#agregar_item").on("click", function () {
        ...
        localStorage.setItem('items_nd', JSON.stringify(items));
    ...
    $("#checkIvaRete1").on("change", function () {
        if ($(this).prop("checked")) {
            ...
            reteIva1 = 0;
            localStorage.removeItem("reteIva1_nd")
        }
        // Guardar items en localStorage
        calcular_totales();
    });
    ...
    $("#checkReteRenta").on("change", function () {
        if ($(this).prop("checked")) {
            ...
            localStorage.setItem("reteRenta_nd", reteRenta);
        } else {
            reteRenta = 0;
            localStorage.removeItem("reteRenta_nd");
        }
        calcular_totales();
    });
    ...
    $("#checkIvaPerci1").on("change", function () {
        if ($(this).prop("checked")) {
            ...
            localStorage.setItem("perciIva1_nd", perciIva1);
        } else {
            perciIva1 = 0;
            localStorage.removeItem("perciIva1_nd");
        }
        calcular_totales();
    });
    ...
    $("#cancelarDTE").on("click", function () {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                localStorage.removeItem("items_nd")
                localStorage.removeItem("reteIva1_nd")
                localStorage.removeItem("reteRenta_nd")
...
function generar_documento() {
    ...
    $.ajax({
        url: "/business/factura?dte=nota_debito",
        method: "POST",
            ...
            if (response.status == 201) {
                if (response.message.estado == "PROCESADO") {
                    Swal.fire({
                        ...
                        text: 'La Nota de Débito ha sido generada exitosamente',
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'Nota de Débito generada en CONTINGENCIA',
                        text: 'Se generó la Nota de Débito, pero no se envió a MH',
                        showConfirmButton: false,
                        timer: 2000
                    }).then(() => {
                        $("#loadingOverlay").addClass("d-none")
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Nota de Débito Rechazada',
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'La Nota de Débito no se envió',
        ...
        error: function (error) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Ha ocurrido un error al generar la Nota de Débito',
                showConfirmButton: false,
                timer: 2000
            })
        }
